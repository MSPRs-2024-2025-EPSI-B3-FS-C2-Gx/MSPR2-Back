...
# 2) Modèle GlobalTotalCase 
class GlobalTotalCase(db.Model):
    __tablename__ = 'global_total_case'
    
    year = db.Column("Year", db.Integer, primary_key=True)
    total_cumulative_cases = db.Column("total_cumulative_cases", db.Float)
    year_ts = db.Column("Year_ts", db.DateTime)
    
    def to_dict(self):
        return {
            'year': self.year,
            'total_cumulative_cases': self.total_cumulative_cases,
            'year_ts': self.year_ts.isoformat() if self.year_ts else None
        }


# 3) Modèle RegionYearlySummary
...
# CRUD Pour GlobalTotalCase

@app.route('/global_total_case', methods=['GET'])
def get_all_global_total_cases():
    """
    Récupère la liste de tous les enregistrements de la table global_total_case.
    ---
    tags:
      - Global Total Case
    responses:
      200:
        description: Liste de tous les enregistrements
    """
    records = GlobalTotalCase.query.all()
    return jsonify([r.to_dict() for r in records]), 200

@app.route('/global_total_case/<int:year>', methods=['GET'])
def get_global_total_case(year):
    """
    Récupère un enregistrement par le champ 'year'.
    ---
    tags:
      - Global Total Case
    parameters:
    ...
    responses:
      ...
      404:
        description: Enregistrement non trouvé
    """
    record = GlobalTotalCase.query.get_or_404(year)
    return jsonify(record.to_dict()), 200

@app.route('/global_total_case', methods=['POST'])
def create_global_total_case():
    """
    Crée un nouvel enregistrement dans la table global_total_case.
    ---
    tags:
      - Global Total Case
    parameters:
      - name: body
        in: body
        required: true
        description: Données de l'année globale
        schema:
          type: object
          required:
            - year
    ...
    if not data or 'year' not in data:
        return jsonify({"error": "Le champ 'year' est obligatoire"}), 400

    new_record = GlobalTotalCase(
        year=data['year'],
        total_cumulative_cases=data.get('total_cumulative_cases', 0)
    )
    db.session.add(new_record)
    db.session.commit()
    return jsonify({"message": "Enregistrement créé avec succès"}), 201

@app.route('/global_total_case/<int:year>', methods=['PUT'])
def update_global_total_case(year):
    """
    Met à jour un enregistrement existant dans la table global_total_case.
    ---
    tags:
      - Global Total Case
    parameters:
      - name: year
        in: path
        description: Année à mettre à jour
        required: true
        schema:
          type: integer
      - name: body
    ...
    if 'total_cumulative_cases' in data:
        record.total_cumulative_cases = data['total_cumulative_cases']

    db.session.commit()
    return jsonify({"message": "Enregistrement mis à jour avec succès"}), 200

@app.route('/global_total_case/<int:year>', methods=['DELETE'])
def delete_global_total_case(year):
    """
    Supprime un enregistrement de la table global_total_case.
    ---
    tags:
      - Global Total Case
    parameters:
